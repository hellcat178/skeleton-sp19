...
public abstract class Creature extends Occupant {
    ...
    /** Imitate above method, define a method to find creatures*/
    public List<Creature> getNeighborsCreatureOfType(Map<Direction, Occupant> n,
                                              String type) {
        List<Creature> L = new ArrayList<>();
        for (Map.Entry<Direction, Occupant> entry : n.entrySet()) {
            String occupantName = entry.getValue().name();
            if (occupantName.equals(type)) {
                L.add( (Creature) entry.getValue());
            }
        }
        return L;
    }

    /** Imitate above method, define a method to find creatures's direction*/
    public List<Direction> getNeighborsCreatureDirectionOfType(Map<Direction, Occupant> n, String type) {
